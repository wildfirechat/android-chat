...
//            CrashReport.initCrashReport(getApplicationContext(), BUGLY_ID, false);
        ...
        if (getCurProcessName(this).equals(BuildConfig.APPLICATION_ID)) {
            ...
            //满足国密环境，需要服务同步开启，普通用户不要使用，无法中途切换，需要一开始确认是否需要此功能。
            //ChatManager.Instance().useSM4();
            //ChatManager.Instance().setDataVerify(true);
            //ChatManager.Instance().setEncryptMaxMinutes(5);

//             // 设备备选地址及网络策略
//             ChatManager.Instance().setBackupAddress("101.35.103.221", 443);
//             //  网络策略，0是复合连接；1是使用主要网络；2使用备选网络
//             ChatManager.Instance().setBackupAddressStrategy(1);
            ...
            // 若用户开启了后台保活，则启动前台服务
            SharedPreferences keepAliveSp = getSharedPreferences(Config.SP_CONFIG_FILE_NAME, Context.MODE_PRIVATE);
            if (keepAliveSp.getBoolean(KeepAliveService.PREF_KEY_KEEP_ALIVE, false)) {
