 ...
 * 接龙参与条目类
 * <p>
 * 表示一个用户对接龙的参与记录，包含用户ID、参与内容、时间等信息。
 * 与iOS的WFCCCollectionEntry对应。
 * </p>
 *
 * @author WildFireChat
 * @since 2020
 */
public class CollectionEntry implements Parcelable {
    /**
     * 条目ID
     */
     ...
     * 转换为JSON对象
     * <p>
     * 注意：只包含与iOS兼容的字段（userId, content, createdAt）
     *
     * @return JSON对象
    ...
    protected CollectionEntry(Parcel in) {
    ...
    public static final Creator<CollectionEntry> CREATOR = new Creator<CollectionEntry>() {
        @Override
        public CollectionEntry createFromParcel(Parcel source) {
            return new CollectionEntry(source);
        }

        @Override
        public CollectionEntry[] newArray(int size) {
